...
fn get_module_main() -> &'static [u8] {
    include_bytes!("../../../../dist/main/index.bin")
}

#[napi]
pub fn load(env: Env, module: JsObject, out_require: JsFunction) -> Result<JsUnknown, Error> {
    configure_v8(&env, &module)?;
    let __filename = module
        .get::<&str, JsString>("filename")?
        .expect("__filename not exist");
    let __dirname = module
        .get::<&str, JsString>("path")?
        .expect("__dirname not exist");
    ...
    let js_module_function = unsafe { js_module_function.cast::<JsFunction>() };
    let modules_unknown = module.into_unknown();
    js_module_function.call(
        None,
        &[
            exports.into_unknown(),
            require,
            modules_unknown,
            __filename.into_unknown(),
            __dirname.into_unknown(),
        ],
    )
}
